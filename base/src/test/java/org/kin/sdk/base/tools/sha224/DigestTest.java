        ...
        extends SimpleTest
{
    ...
    public void performTest() throws DecoderException {
        ...
        vectorTest(digest, input.length - 1, resBuf, lastV, Hex.decodeHex(results[input.length - 1]));
    ...
    private void testEncodedState(byte[] resBuf, byte[] input, byte[] expected)
    {
        ...
        {
            fail("failing state vector test", expected, new String(Hex.encodeHex(resBuf)));
        }
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing state copy1 vector test", expected, new String(Hex.encodeHex(resBuf)));
        }
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing state copy2 vector test", expected, new String(Hex.encodeHex(resBuf)));
        }
    }
    ...
    private void testMemo(byte[] resBuf, byte[] input, byte[] expected)
    {
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing memo vector test", results[results.length - 1], new String(Hex.encodeHex(resBuf)));
        }
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing memo reset vector test", results[results.length - 1], new String(Hex.encodeHex(resBuf)));
        }
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing memo copy vector test", results[results.length - 1], new String(Hex.encodeHex(resBuf)));
        }
    }
    ...
    private void testClone(byte[] resBuf, byte[] input, byte[] expected)
    {
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing clone vector test", results[results.length - 1], new String(Hex.encodeHex(resBuf)));
        }
        ...
        if (!areEqual(expected, resBuf))
        {
            fail("failing second clone vector test", results[results.length - 1], new String(Hex.encodeHex(resBuf)));
        }
    }
            ...
            byte[] expected)
    {
        ...
        {
            fail("Vector " + count + " failed got " + new String(Hex.encodeHex(resBuf)));
        }
    }
            ...
            byte[] expected)
    {
        ...
        {
            fail("Offset " + count + " failed got " + new String(Hex.encodeHex(resBuf)));
        }
    }
    ...
    protected void millionATest(
            String expected) throws DecoderException {
        byte[] resBuf = new byte[digest.getDigestSize()];
        ...
        if (!areEqual(resBuf, Hex.decodeHex(expected)))
        {
            fail("Million a's failed", expected, new String(Hex.encodeHex(resBuf)));
        }
    }
    ...
    protected void sixtyFourKTest(
        ...
        if (!areEqual(resBuf, Hex.decodeHex(expected)))
        {
            fail("64k test failed", expected, new String(Hex.encodeHex(resBuf)));
        }
    }
}
