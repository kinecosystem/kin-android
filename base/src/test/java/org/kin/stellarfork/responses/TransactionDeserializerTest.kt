...
import org.kin.stellarfork.MemoHash
import org.kin.stellarfork.MemoNone
...
class TransactionDeserializerTest {
    ...
    fun testDeserialize() {
        ...
        assertEquals(transaction.createdAt, "2015-11-20T17:01:28Z")
        assertEquals(transaction.pagingToken, "3933090531512320")
        assertEquals(
            transaction.sourceAccount.accountId,
            "GCUB7JL4APK7LKJ6MZF7Q2JTLHAGNBIUA7XIXD5SQTG52GQ2DAT6XZMK"
        )
        assertEquals(transaction.sourceAccountSequence, 2373051035426646L)
        assertEquals(transaction.feePaid, 100)
        assertEquals(transaction.operationCount, 1)
        assertEquals(
        ...
        assertTrue(transaction.getMemo() is MemoHash)
        val memo = transaction.getMemo() as MemoHash?
        assertEquals(
            "51041644e83d6ac868c849418b6392ddbe9df53f000000000000000000000000",
            memo!!.hexValue
        )
        assertEquals(
        ...
        assertEquals(transaction.links.ledger.href, "/ledgers/915744")
        assertEquals(
    ...
    @Test
    fun testDeserializeWithoutMemo() {
        val transaction = instance!!.fromJson(
            jsonMemoNone,
            TransactionResponse::class.java
        )
        assertTrue(transaction.getMemo() is MemoNone)
    }
