...
class AccountsPageDeserializerTest {
    @Test
    fun testDeserialize() {
        val response = instance!!.fromJson<Page<AccountResponse>>(
            json,
            object :
                TypeToken<Page<AccountResponse?>?>() {}.type
        )
        assertEquals(
            response.records[0].keypair.accountId,
            "GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN7"
        )
        assertEquals(response.records[0].pagingToken, "1")
        assertEquals(
            response.records[9].keypair.accountId,
        ...
        assertEquals(
            ...
            "/accounts?order=desc&limit=10&cursor=1"
        )
        assertEquals(response.links!!.self.href, "/accounts?order=asc&limit=10&cursor=")
    }
