...
class LedgerPageDeserializerTest {
    @Test
    fun testDeserialize() {
        val response = instance!!.fromJson<Page<LedgerResponse>>(
            json,
            object : TypeToken<Page<LedgerResponse?>?>() {}.type
        )
        assertEquals(
            response.records[0].hash,
            "f6fc9a29b5ecec90348e17a10a60e019c5cb8ea24f66a1063e92c13391b09b8d"
        )
        assertEquals(response.records[0].pagingToken, "3862202096287744")
        assertEquals(
            response.records[9].hash,
        ...
        assertEquals(
            response.links!!.prev.href,
            "/ledgers?order=asc&limit=10&cursor=3862202096287744"
        )
        assertEquals(response.links!!.self.href, "/ledgers?order=desc&limit=10&cursor=")

        assertEquals(
            response.records[0], LedgerResponse(
                899239,
                "f6fc9a29b5ecec90348e17a10a60e019c5cb8ea24f66a1063e92c13391b09b8d",
                "3862202096287744",
                "0bc807d3be86e8a9f7c913bc0e408f67402f50fbf4c0ea6583bc0f788075a567",
                0,
                0,
                "2015-11-19T22:01:34Z",
                "101343867604.8975480",
                "1908.2251218",
                0,
                "10.0000000",
                50,
                LedgerResponse.Links(
                    Link("/ledgers/899239/effects{?cursor,limit,order}", true),
                    Link("/ledgers/899239/operations{?cursor,limit,order}", true),
                    Link("/ledgers/899239", false),
                    Link("/ledgers/899239/transactions{?cursor,limit,order}", true)
                )
            )
        )
    }
