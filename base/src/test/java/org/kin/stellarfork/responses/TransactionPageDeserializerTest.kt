...
class TransactionPageDeserializerTest {
    @Test
    fun testDeserialize() {
        val response = instance!!.fromJson<Page<TransactionResponse>>(
            json,
            object :
                TypeToken<Page<TransactionResponse?>?>() {}.type
        )
        assertEquals(
            response.records[0].sourceAccount.accountId,
            "GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN7"
        )
        assertEquals(response.records[0].pagingToken, "12884905984")
        assertTrue(response.records[0].getMemo() is MemoText)
        var memoText = response.records[0].getMemo() as MemoText?
        assertEquals(memoText!!.text, "hello world")
        assertEquals(
            response.records[0].links.account.href,
            "/accounts/GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN7"
        )
        assertEquals(
            response.records[9].sourceAccount.accountId,
            "GAENIE5LBJIXLMJIAJ7225IUPA6CX7EGHUXRX5FLCZFFAQSG2ZUYSWFK"
        )
        // Empty memo_text
        assertTrue(response.records[2].getMemo() is MemoText)
        memoText = response.records[2].getMemo() as MemoText?
        assertEquals(memoText!!.text, "")
        assertEquals(
