 ...
 * @since 1.4
 */
object Base64TestData {
    const val CODEC_101_MULTIPLE_OF_3 = "124"
    const val CODEC_98_NPE =
        "YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXpBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWjAxMjM"
    const val CODEC_98_NPE_DECODED =
...
// to further parameterize commons-codec to support this pointless variation.
    const val ENCODED_64_CHARS_PER_LINE =
    ...
    const val ENCODED_76_CHARS_PER_LINE =
     ...
     */
    fun randomData(size: Int, urlSafe: Boolean): Array<ByteArray?> {
        val r = Random()
        val decoded = ByteArray(size)
        r.nextBytes(decoded)
        val encoded =
            if (urlSafe) encodeBase64URLSafe(decoded) else encodeBase64(
                decoded
            )
