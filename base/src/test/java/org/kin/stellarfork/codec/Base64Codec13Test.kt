...
class Base64Codec13Test {
    companion object {
        private val STRINGS = arrayOfNulls<String>(181)
        private val CHUNKED_STRINGS =
            arrayOfNulls<String>(STRINGS.size)
        private val BYTES =
            arrayOfNulls<ByteArray>(STRINGS.size)
        ...
        private fun initCHUNKED_STRINGS() {
            val c = CHUNKED_STRINGS
            c[0] = ""
            c[1] = "uA==\r\n"
            c[2] = "z9w=\r\n"
            c[3] = "TQ+Z\r\n"
        ...
        private fun initBYTES() {
            ...
            b[0] = byteArrayOf()
            b[1] = byteArrayOf(-72)
            b[2] = byteArrayOf(-49, -36)
            b[3] = byteArrayOf(77, 15, -103)
            b[4] = byteArrayOf(110, 24, -44, 96)
            b[5] = byteArrayOf(-43, -61, -67, -75, -33)
            b[6] = byteArrayOf(-80, -52, 71, -96, -105, -7)
            b[7] = byteArrayOf(-115, 7, 15, -108, 59, 25, -49)
            b[8] = byteArrayOf(76, 6, -113, -99, -11, -65, 9, -123)
            b[9] = byteArrayOf(113, 116, -39, -69, 12, -41, 60, -29, 82)
            b[10] = byteArrayOf(46, -39, -1, 101, 53, 120, 34, 52, -6, 56)
            b[11] = byteArrayOf(-23, -8, 126, -115, -43, 55, -43, 35, -90, 90, -73)
            b[12] = byteArrayOf(19, -2, 76, -96, 62, 42, 10, -16, -18, 77, -63, 64)
            b[13] =
                byteArrayOf(-38, 127, 88, -55, -16, -116, -56, -59, -34, -103, -69, 108, -79)
            b[14] =
                byteArrayOf(-88, 25, 26, -36, 26, -70, 87, 125, 71, -103, -55, -121, -114, 70)
            b[15] =
                byteArrayOf(90, -4, -103, 123, -87, 16, -14, -100, -69, -101, 110, 110, 113, -84, 9)
            b[16] = byteArrayOf(
                -95,
                -118,
                113,
        ...
        private fun utf8(s: String?): ByteArray? { // We would use commons-codec-1.4.jar own utility method for this, but we
...
// duplication here.
    ...
    fun testEncoder() {
        ...
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                ...
                val binary = BYTES[i]
                val b =
                    Arrays.equals(base64, enc.encode(binary) as ByteArray?)
                Assert.assertTrue("Encoder test-$i", b)
            }
        }
    }
    ...
    fun testDecoder() {
        ...
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                ...
                val binary = BYTES[i]
                val b =
                    Arrays.equals(binary, dec.decode(base64) as ByteArray?)
                Assert.assertTrue("Decoder test-$i", b)
            }
        }
    }
    ...
    fun testBinaryEncoder() {
        val enc: BinaryEncoder = Base64()
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                val base64 =
                    utf8(STRINGS[i])
                val binary = BYTES[i]
                val b = Arrays.equals(base64, enc.encode(binary))
                Assert.assertTrue("BinaryEncoder test-$i", b)
            }
        }
    }
    ...
    @Test
    @Throws(DecoderException::class)
    fun testBinaryDecoder() {
        val dec: BinaryDecoder = Base64()
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                val base64 =
                    utf8(STRINGS[i])
                val binary = BYTES[i]
                val b = Arrays.equals(binary, dec.decode(base64))
                Assert.assertTrue("BinaryDecoder test-$i", b)
            }
        }
    }
    ...
    fun testStaticEncode() {
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                ...
                Assert.assertTrue("static Base64.encodeBase64() test-$i", b)
            }
        }
    }
    ...
    fun testStaticDecode() {
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                ...
                Assert.assertTrue("static Base64.decodeBase64() test-$i", b)
            }
        }
    }
    ...
    @Test
    @Throws(EncoderException::class)
    fun testStaticEncodeChunked() {
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                val base64Chunked = utf8(
                    CHUNKED_STRINGS[i]
                )
                val binary = BYTES[i]
                val b = Arrays.equals(
                    base64Chunked,
                    encodeBase64Chunked(binary)
                )
                Assert.assertTrue("static Base64.encodeBase64Chunked() test-$i", b)
            }
        }
    }
    ...
    fun testStaticDecodeChunked() {
        for (i in STRINGS.indices) {
            if (STRINGS[i] != null) {
                ...
                val b = Arrays.equals(
                    binary,
                    decodeBase64(base64Chunked)
                )
                Assert.assertTrue("static Base64.decodeBase64Chunked() test-$i", b)
            }
        }
    }
}
