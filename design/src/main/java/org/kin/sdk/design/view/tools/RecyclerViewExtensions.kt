...
import org.kin.sdk.design.view.widget.internal.HeaderView
import org.kin.sdk.design.view.widget.internal.PlaceholderView
...
object RecyclerViewTools {
    fun header(@StringRes title: Int): RecyclerHeaderItem =
        RecyclerHeaderItem(title)
    fun placeholder(@StringRes title: Int): RecyclerPlaceholderItem =
        RecyclerPlaceholderItem(title)
}
            ...
            currentRecyclerView.post {
                if (currentItems != null) {
                    val diffResult = DiffUtil.calculateDiff(
                        ListDiffCallback<Any>(
                            currentItems as List<Any>,
                            items as List<Any>
                        )
                    )
                    currentItems = items
...
class RecyclerViewBuilderContext {
    ...
    internal fun onBindView(@Suppress("UNUSED_PARAMETER") context: Context, viewType: Int, view: View, data: Any) {
    ...
    inner class LayoutCreator<ViewType : View, ItemViewModelType> {
        ...
        fun toLayout(clzz: Class<*>): Layout =
            Layout(
                clzz,
                { context ->
...
fun RecyclerView.build(builder: RecyclerViewBuilderContext.() -> Unit) {
    val builderContext =
        RecyclerViewBuilderContext()

    builderContext.builder()

    adapter = ListBackedRecyclerViewAdapter(
        builderContext
    )
}
