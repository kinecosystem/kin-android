...
class LineItemListItemView(context: Context) : LinearLayout(context) {

    private val textStack = with(LinearLayout(context)) {
        orientation = VERTICAL

        addTo(this@LineItemListItemView, LinearLayout.LayoutParams(
            0,
    ...
    var sku: String = ""
        set(value) {
            if (value.isBlank()) {
                skuView.visibility = View.GONE
            } else {
                skuView.visibility = View.VISIBLE
            }
            skuView.text = value
            field = value
        }
