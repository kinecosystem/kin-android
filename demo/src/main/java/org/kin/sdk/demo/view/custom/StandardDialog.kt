...
class StandardDialog(context: Context) : Dialog(context) {
    ...
    companion object {
        fun confirm(context: Context, title: String = "", description: String = "", confirmed: (Boolean) -> Unit) {
            ...
            with(DialogButton(context)) {
                setText(context.getString(R.string.button_cancel))
                setOnClickListener {
                    standardDialog.dismiss()
                    confirmed(false)
                }
    ...
    init {
        rootLayout = LinearLayout(context).also { rootLayout ->
            ...
            titleText = with(PrimaryTextView(context)) {
                setText(title)
                textSize = 22f
            ...
            descriptionText = with(SecondaryTextView(context)) {
                setText(description)
                visibility = GONE
            ...
            errorText = with(CodeStyleTextView(context)) {
                setText(error)
                visibility = GONE

                val scrollWrapper = with(MaxHeightScrollView(context)) {
                    maxHeight = 120.dip
